(function () {
  ...
  function injectEnhancementStyles() {
    ...
    style.textContent = `
      ...
      body.bw-builder-modal-open{overflow:hidden!important;overscroll-behavior:none}
      #bw_builder_sheet{display:none;position:fixed;inset:0;overflow:hidden;overscroll-behavior:none;background:rgba(12,27,51,.45);z-index:1800}
      #bw_builder_sheet.on{display:block}
      #bw_builder_sheet .bw-sheet{position:absolute;left:0;right:0;bottom:0;max-height:72vh;max-height:72dvh;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;
      ...
      .bw-sheet-sticky{position:sticky;top:-14px;z-index:8;margin:-14px -14px 12px;padding:14px 14px 10px;background:rgba(255,255,255,.97);border-bottom:1px solid #E1E9F5;backdrop-filter:blur(10px)}
      .bw-sheet-head{display:flex;align-items:center;gap:10px;margin-bottom:9px}
      ...
      .bw-recovery-bar{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.bw-recovery-bar button{min-height:40px;border:1px solid #C9D7ED;border-radius:10px;background:#F6F9FF;color:#20386A;font:800 9px 'Space Mono'}.bw-recovery-bar button:first-child{background:#E4F7EC;border-color:#B8E2C8;color:#17623B}.bw-recovery-bar button:last-child{background:#FDECEF;border-color:#F3B6C2;color:#A22F49}
      ...
      @media(max-width:560px){#bw_builder_sheet .bw-sheet{max-height:88vh;max-height:88dvh;border-radius:20px 20px 0 0}.bw-sheet-head button{width:44px;height:44px}.bw-recovery-bar button{min-height:44px}}
  ...
  function injectBuilderStabilityStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:560px){
        #bscr .top{position:relative;z-index:45;display:grid;grid-template-columns:38px minmax(42px,1fr) 43px 44px 48px 54px;
          gap:4px;padding:8px max(6px,env(safe-area-inset-right)) 7px max(6px,env(safe-area-inset-left));background:rgba(237,243,252,.97)}
        #bscr .top .sp{display:none}
        #bscr .top .bw-builder-back{grid-column:1}
        #bscr .top .bw-official-logo{grid-column:2;width:min(74px,100%)!important;height:28px!important;justify-self:start}
        #bscr .top #bd_undoBtn{grid-column:3}
        #bscr .top .bw-clear-tool{display:block!important;grid-column:4}
        #bscr .top .bw-top-tool{grid-column:5}
        #bscr .top #bd_expBtn{grid-column:6}
        #bscr .top .bw-save-tool,#bscr .top .bw-mobile-secondary{display:none!important}
        #bscr .top .ghost,#bscr .top .exp{min-width:0;width:100%;padding:8px 3px!important;font-size:8.5px!important;white-space:nowrap}
      ...
      @media(max-width:345px){
        ...
        #bscr .top .bw-official-logo{width:52px!important}
        #bscr .top .ghost,#bscr .top .exp{font-size:8px!important;padding-left:2px!important;padding-right:2px!important}
      }
  ...
  function clearBuilderCanvasSafely() {
    if (!window.__bapi || !window.__bapi.count()) {
      toast('the canvas is already clear', 'warn');
      return;
    }
    ...
    scheduleBuilderSave();
    renderBuilderSheet();
    const undo = document.getElementById('bd_undoBtn');
    if (undo) {
      undo.classList.add('bw-undo-ready');
      setTimeout(() => undo.classList.remove('bw-undo-ready'), 3600);
    }
    toast('canvas cleared · undo restores it', 'ok');
  }
  ...
  function importBuilderState(data, format, background, selectedIndex, options) {
    const opts=options||{};
    ...
    suppressBuilderSave = true;
    if(opts.skipHistory)window.__bapiHistorySuspend=true;
    try {
      window.__bapi.importEls(clean, format || window.__bapi.fmt()[0], background || '');
    } finally {
      window.__bapiHistorySuspend=false;
    }
    suppressBuilderSave = false;
  ...
  function renderBuilderSheet() {
    ...
    host.innerHTML = '<div class="bw-sheet-sticky"><div class="bw-sheet-head"><div><b>builder controls</b><br><small>' + state.data.length +
      ' editable layers · autosave on</small></div><button type="button" data-close aria-label="Close builder controls">×</button></div>' +
      '<div class="bw-recovery-bar"><button type="button" data-a="undo-history">undo last</button><button type="button" data-a="reset-original">reset design</button><button type="button" data-a="clear-canvas">clear canvas</button></div></div>' +
      ...
      '<button class="bw-ctrl" data-a="restore">restore autosave</button><button class="bw-ctrl" data-a="save-library">save to library</button></div>';
    host.querySelector('[data-close]').onclick = () => closeBuilderSheet(sheet);
    ...
    bind('[data-a="undo-history"]',()=>{const button=document.getElementById('bd_undoBtn');if(button&&!button.disabled)button.click();else toast('nothing to undo yet','warn');});
    ...
    bind('[data-a="save-library"]',()=>{if(typeof window.saveComp==='function')window.saveComp();closeBuilderSheet(sheet);});
    bind('[data-a="clear-canvas"]',()=>{clearBuilderCanvasSafely();closeBuilderSheet(sheet);});
  }

  function closeBuilderSheet(sheet) {
    if(!sheet)return;
    sheet.classList.remove('on');
    sheet.setAttribute('aria-hidden','true');
    document.body.classList.remove('bw-builder-modal-open');
    const trigger=builderSheetTrigger;
    builderSheetTrigger=null;
    if(trigger&&document.contains(trigger)&&trigger.offsetParent!==null&&typeof trigger.focus==='function')requestAnimationFrame(()=>trigger.focus({preventScroll:true}));
  }
  ...
  function openBuilderSheet() {
    ...
    if (!sheet) {
      ...
      sheet.setAttribute('role','dialog');sheet.setAttribute('aria-modal','true');sheet.setAttribute('aria-label','Builder layers and controls');sheet.setAttribute('aria-hidden','true');
      sheet.innerHTML = '<div class="bw-sheet"></div>';
      sheet.onclick = event => { if (event.target === sheet) closeBuilderSheet(sheet); };
      sheet.onkeydown=event=>{
        if(event.key==='Escape'){event.preventDefault();closeBuilderSheet(sheet);return;}
        if(event.key!=='Tab')return;
        const controls=Array.from(sheet.querySelectorAll('button:not([disabled]),input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(node=>node.offsetParent!==null);
    ...
    builderSheetTrigger=document.activeElement;
    sheet.classList.add('on');
    sheet.setAttribute('aria-hidden','false');
    document.body.classList.add('bw-builder-modal-open');
    renderBuilderSheet();
    const close=sheet.querySelector('[data-close]');if(close)requestAnimationFrame(()=>close.focus({preventScroll:true}));
  }
  ...
  let creativeV5Trigger = null;
  let creativeV5ReturnToWriter = false;

  function creativeV5PageSize() {
    return window.matchMedia && window.matchMedia('(max-width: 560px)').matches ? 12 : 24;
  }
  ...
  function closeWorkshopAfterMobileAdd() {
    if (!window.matchMedia || !window.matchMedia('(max-width: 560px)').matches) return;
    const sheet=document.getElementById('bw_creative_v5');
    if(sheet) closeCreativeKitV5(sheet,{keepBuilder:true});
  }
  ...
  function useV5Design(item) {
    ...
    if(creativeV5Mode==='overlay') {
      ...
      if(atomicAddV5(overlay,item.name))closeWorkshopAfterMobileAdd(); return;
    }
    ...
    importBuilderState(layers,format[0],'transparent',layers.length-1);
    renderBuilderSheet();
    const sheet=document.getElementById('bw_creative_v5'); if(sheet) closeCreativeKitV5(sheet,{keepBuilder:true});
  ...
  function useV5Asset(kind,item) {
    ...
    if(atomicAddV5(layers,item.name))closeWorkshopAfterMobileAdd();
  }
  ...
  function installCreativeV5Styles() {
    ...
    const style=document.createElement('style'); style.id='bw_creative_v5_style'; style.textContent=`
      body.bw-modal-open{overflow:hidden!important;overscroll-behavior:none}
      #bw_creative_v5{display:none;position:fixed;inset:0;width:100vw;height:100vh;height:100dvh;max-width:100%;z-index:2500;overflow:hidden;background:rgba(7,17,36,.63);align-items:flex-end;justify-content:center;backdrop-filter:blur(5px)}
      #bw_creative_v5.on{display:flex}#bw_creative_v5,#bw_creative_v5 *{box-sizing:border-box}
      #bw_creative_v5 .bw-v5-shell{width:min(860px,100vw);max-width:100vw;max-height:94vh;max-height:94dvh;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;background:#F7FAFF;border-radius:26px 26px 0 0;box-shadow:0 -28px 70px rgba(7,17,36,.38);padding:0 14px calc(18px + env(safe-area-inset-bottom))}
      ...
      .bw-v5-options{display:flex;min-width:0;align-items:center;justify-content:space-between;gap:9px;margin:8px 0}.bw-v5-themes{min-width:0;flex:1}.bw-v5-modes{display:flex;flex:0 0 auto;gap:5px}.bw-v5-help{color:#7084A9;font:650 8px/1.4 'Space Mono'}
      ...
      .bw-v5-use{display:block;min-width:0;width:100%;border:0;background:#fff;padding:8px;text-align:left;touch-action:manipulation}.bw-v5-preview{position:relative;width:100%;aspect-ratio:1/1.05;border-radius:11px;overflow:hidden;background:#EAF1FF}.bw-v5-art{display:block;width:100%;height:100%}
      ...
      @media(min-width:700px){.bw-v5-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
      @media(max-width:560px){
        #bw_creative_v5{align-items:stretch;background:#F7FAFF;backdrop-filter:none}
        #bw_creative_v5 .bw-v5-shell{width:100vw;max-width:100vw;height:100vh;height:100dvh;max-height:none;border-radius:0;padding:0 10px calc(14px + env(safe-area-inset-bottom))}
        .bw-v5-head{margin:0 -10px;padding:calc(11px + env(safe-area-inset-top)) 10px 11px;gap:8px}
        .bw-v5-head img{width:72px;height:25px}.bw-v5-head b{font-size:13px}.bw-v5-head small{font-size:7px;line-height:1.35}
        .bw-v5-close{width:44px;height:44px;flex:0 0 44px}
        .bw-v5-toolbar{top:calc(66px + env(safe-area-inset-top));margin:0 -1px 8px;padding:7px 1px}
        .bw-v5-search{display:grid;grid-template-columns:minmax(0,1fr) auto}.bw-v5-count{min-width:62px;justify-content:center}
        .bw-v5-options{align-items:stretch;flex-direction:column;gap:7px}.bw-v5-themes{width:100%;flex:0 0 auto}.bw-v5-modes{align-self:flex-start}
        .bw-v5-tabs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));overflow:visible;gap:6px}
        .bw-v5-tab{min-width:0;white-space:normal;line-height:1.25}.bw-v5-tab,.bw-v5-filter,.bw-v5-theme,.bw-v5-mode{min-height:40px;padding:8px 8px}
        .bw-v5-fave{width:44px;height:44px;right:7px;top:7px;border-radius:12px}
        .bw-v5-grid{gap:8px}.bw-v5-use{padding:7px}.bw-v5-meta{padding-bottom:5px}
      }
      @media(max-width:420px){.bw-v5-proof{grid-template-columns:repeat(3,minmax(0,1fr))}.bw-v5-grid{grid-template-columns:1fr}.bw-v5-preview{aspect-ratio:1/.82}.bw-v5-meta span{min-height:0}}
      @media(max-width:345px){.bw-v5-head small{max-width:150px}}
    `; document.head.appendChild(style);
  }

  function closeCreativeKitV5(sheet, options) {
    if (!sheet) return;
    const opts=options||{};
    const returnToWriter=creativeV5ReturnToWriter&&!opts.keepBuilder;
    sheet.classList.remove('on');
    sheet.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('bw-modal-open');
    if(sheet.dataset.historyPushed==='1'&&!opts.fromPopstate){
      sheet.dataset.historyPushed='0';
      history.back();
    }
    const trigger = creativeV5Trigger;
    creativeV5Trigger = null;
    creativeV5ReturnToWriter=false;
    if(returnToWriter&&typeof window.goWriter==='function')window.goWriter();
    let focusTarget=trigger;
    const activeScreen=focusTarget&&focusTarget.closest&&focusTarget.closest('.screen');
    if(!focusTarget||!document.contains(focusTarget)||focusTarget.offsetParent===null||(activeScreen&&!activeScreen.classList.contains('on'))){
      focusTarget=document.querySelector('#bscr.on .bw-creative-rail,#bscr.on .bw-builder-back,#wscr.on button');
    }
  ...
  function renderV5Filters(sheet,items) {
    ...
    filters.querySelectorAll('[data-v5-filter]').forEach(button=>button.onclick=()=>{creativeV5Filter=button.dataset.v5Filter;creativeV5Limit=creativeV5PageSize();renderV5Catalog(sheet);});
  }
  ...
  function renderV5Catalog(sheet) {
    ...
    sheet.querySelector('#bw_v5_grid').innerHTML=cards+(matches.length>visible.length?'<button class="bw-v5-more" data-v5-more>show '+Math.min(creativeV5PageSize(),matches.length-visible.length)+' more →</button>':'')+(!matches.length?'<div class="bw-v5-empty">No systems match that search yet.</div>':'');
    ...
    const more=sheet.querySelector('[data-v5-more]'); if(more) more.onclick=()=>{creativeV5Limit+=creativeV5PageSize();renderV5Catalog(sheet);};
  }
  ...
  function openCreativeKitV5() {
    ...
    if(!sheet) {
      ...
      sheet.setAttribute('role','dialog'); sheet.setAttribute('aria-modal','true'); sheet.setAttribute('aria-label','Signal Workshop'); sheet.setAttribute('aria-hidden','true');
      ...
      sheet.onclick=event=>{if(event.target===sheet||event.target.closest('[data-v5-close]'))closeCreativeKitV5(sheet);};
      sheet.onkeydown=event=>{
        if(event.key==='Escape'){event.preventDefault();closeCreativeKitV5(sheet);return;}
        if(event.key!=='Tab')return;
        const controls=Array.from(sheet.querySelectorAll('button:not([disabled]),input:not([disabled]),textarea:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter(node=>node.offsetParent!==null);
      ...
      window.addEventListener('popstate',()=>{
        ...
        sheet.dataset.historyPushed='0';
        closeCreativeKitV5(sheet,{fromPopstate:true});
      });
      sheet.querySelector('#bw_v5_search').oninput=()=>{creativeV5Limit=creativeV5PageSize();renderV5Catalog(sheet);};
      sheet.querySelectorAll('[data-v5-tab]').forEach(button=>button.onclick=()=>{creativeV5Tab=button.dataset.v5Tab;creativeV5Filter='all';creativeV5Limit=creativeV5PageSize();sheet.querySelectorAll('[data-v5-tab]').forEach(node=>node.classList.toggle('on',node===button));renderV5Catalog(sheet);});
    ...
    creativeV5Trigger=document.activeElement;
    creativeV5Limit=creativeV5PageSize();
    const builderSheet=document.getElementById('bw_builder_sheet'); if(builderSheet)closeBuilderSheet(builderSheet);
    renderV5Catalog(sheet); sheet.classList.add('on'); sheet.setAttribute('aria-hidden','false'); document.body.classList.add('bw-modal-open');
    if(sheet.dataset.historyPushed!=='1'){
      history.pushState(Object.assign({},history.state||{},{cmvngModal:'signal-workshop'}),'',location.href);
      sheet.dataset.historyPushed='1';
    }
    const close=sheet.querySelector('[data-v5-close]'); if(close)requestAnimationFrame(()=>close.focus({preventScroll:true}));
  }
  ...
  function completeFormatChange(event) {
    ...
    importBuilderState(next,target[0],pending.state.background,index,{skipHistory:true});
    renderBuilderSheet();
  ...
  function installBuilderEnhancements() {
    ...
    if (top && exportButton) {
      ...
      topButtons.forEach(button=>{
        ...
        if(action.includes('clearall')) {
          button.classList.add('bw-clear-tool');
          button.setAttribute('aria-label','Clear canvas');
          button.title='Clear canvas (Undo remains available)';
          button.onclick=clearBuilderCanvasSafely;
        }
        if(action.includes('savecomp')) button.classList.add('bw-save-tool');
      });
    ...
    if (!screen.dataset.fullUndoSync) {
      screen.dataset.fullUndoSync = '1';
      document.addEventListener('banger:builder-undo', () => {
        const state = builderState();
        const format = window.__bapi.fmt();
        const clean = state.data.map(item => {
          const copy = Object.assign({}, item);
          delete copy.__id;
          return copy;
        });
        const foundationIndex = clean.findIndex(item => item.type === 'img' && item.src &&
          (+item.x || 0) <= 1 && (+item.y || 0) <= 1 && (+item.w || 0) >= format[1] * .97 &&
          (+item.w || 0) * (+item.ar || 1) >= format[2] * .97);
        foundationSrc = foundationIndex >= 0 ? clean[foundationIndex].src : '';
        foundationLocked = !!foundationSrc;
        const list = window.__bapi.list();
        foundationBuilderId = foundationIndex >= 0 && list[foundationIndex] ? list[foundationIndex].id : null;
        activeCreativeDesignId = '';
        selectedBuilderId = null;
        originalBuilderDesign = clean.length ? {
          data: clone(clean), format: format[0], background: state.background,
          foundationSrc: foundationSrc
        } : null;
        normaliseBuilderSvgrawRenderingV7();
        requestAnimationFrame(applyFoundationLock);
        scheduleBuilderSave();
        renderBuilderSheet();
        updateBuilderMetaV7();
        toast('undo restored layers, format + background', 'ok');
      });
    }
